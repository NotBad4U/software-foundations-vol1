...
(* FIXME: wrong type inference with st Z...
...
Proof.
  ...
  - destruct IHHce1 as [i1 H1].
    ...
    exists (1 + i1 + i2).
    simpl.
    destruct (ceval_step st c1 (i1 + i2)) eqn: Heqn.
    + assert (st' = s).
      { 
    ...
    + apply (ceval_step_more i1 (i1 + i2)) in H1. rewrite H1 in Heqn. discriminate. omega.
  - inversion IHHce as [i H'].
       exists (1 + i). simpl. rewrite H. apply H'.
  - destruct IHHce as [i H'].
    exists (1 + i). simpl. rewrite H. apply H'.
  ...
  - destruct IHHce1 as [i1 H1].
    ...
    exists (1 + i1 + i2).
    simpl. rewrite H.
    destruct (ceval_step st c (i1 + i2)) eqn: Heqn.
      + assert (st' = s).
        {
          apply (ceval_step_more i1 (i1 + i2)) in H1.
          rewrite H1 in Heqn.
        ...
        assumption.
      + apply (ceval_step_more i1 (i1 + i2)) in H1.
        * rewrite H1 in Heqn. discriminate Heqn.
        * omega.
Qed.
